...
A post-hoc test is a follow-up analysis performed after 
finding a significant result in an overall statistical 
test (like ANOVA or Kruskal-Wallis). 
Its purpose is to identify exactly which groups or 
treatments differ from each other. In other words, 
it helps pinpoint where the differences lie between 
multiple groups when the initial test shows that 
...
Once the dataset is ready, head onto the analysis tab in RAISINS and click on browse and upload the data in csv, xls or xlsx format. After uploading select the Treatment and variables of interest(multiple variables can also be selected) ![](images/kwt/Analysis.png)and then click on run analysis. A complete publication ready results and tables will be generated. Results can be downloaded as pdf, html or word format.  
