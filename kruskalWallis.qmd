...
Dr Pratheesh P Gopinath<sup>1</sup>, Jithin Chandran<sup>2</sup>, Mohammed Hisham M<sup>2</sup>

::: affiliations
<sup>1</sup>Department of Agricultural Statistics, Kerala Agricultural University, <sup>2</sup>Statoberry LLP
:::
...
1.  A food scientist is evaluating four drink flavours (V1 to V4) for five sensory attributes: appearance, color, texture, taste, and flavour. Each flavour is rated by ten judges; since each judge gives a score, these scores can be considered as ten replications per group. A Kruskal-Wallis test can be performed for each sensory attribute to check if there are any significant differences in ratings among the flavours. <a href="#example1" class="toggle-image" data-target="example-image1">Example1</a>
...
RAISINS is an innovative online platform that simplifies data analysis for agricultural research—no downloads required. It integrates the power of R, Python, and AI to offer user-friendly, robust statistical tools. The platform is developed by STATOBERRY LLP, with mentorship from the Department of Agricultural Statistics, College of Agriculture, Vellayani, Kerala Agricultural University.
...
We'll guide you through the entire kruskal wallis test. For that let's look how the analysis can be carried out using an Example 1 descibed in @sec-exkwt.

You can see in @fig-raisinkwt how the dataset should be arranged for analysis in RAISINS.

Data preparation in RAISINS is super easy it is well explained in the section @sec-createData. Model datasets for testing from the app is explained in @sec-Datasets.

![Dataset arrangement for Kruskal-Wallis test in RAISINS](images/kwt/ex1.png){#fig-raisinkwt fig-align="center" width="558"}

Once the dataset is ready, head onto the analysis tab in RAISINS and click on browse and upload the data in csv, xls or xlsx format. After uploading select the treatment and variables of interest(multiple variables can also be selected) and then click on run analysis. A complete publication ready results and tables will be generated. Results can be downloaded as pdf, html or word format.

![](images/kwt/Analysis.png)

### Results

![](images/kwt/result.png){#fig-res1 fig-align="center" width="703"}
...
From the results @fig-res1 , it's found that appearance wise, drinks V3 and V4 are the best, and by color V2 and V3 are the best. If your objective is to select treatments based on all characters, a multivariate analysis has to be performed. For that click on multivariate analysis tab. The results as mentioned in @sec-multi gives you the best treatment based on all the observed characters.
</details>

### Customization tabs

Additionally You can also change the decimal points, post-hoc tests, p-adjustment method, font and significance level according to your requirements as shown in @fig-tabs
...
![Connected line plot](images/kwt/Connected.png){fallback="https://via.placeholder.com/400x300?text=Connected+Plot"}

::: {#connectedplot-desc .plot-description}
A **connected line plot**, also known as a connected scatter plot, is a type of chart where data points are represented by markers and connected by lines, visualizing the evolution of a variable over time or another sequential order. It helps to show trends and relationships between two variables as they change across a third variable
:::
::::
:::::
...
A **mean value plot** also known as a line plot, is a graphical representation of the average (mean) value of a dataset, often accompanied by error bars or confidence intervals that indicate the uncertainty or variability around the mean. It's used to visualize the central tendency and spread of data. 
:::
::::
:::::
...
A **violin plot** combines the box plot and the density trace (or smoothed histogram) into a single display that reveals structure found
within the data.
:::
::::
:::::
...
## FAQ's
![](images/kwt/faq.png)
More informations and other Frequently asked questiosn are available in the FAQ's tab

## User
![](images/kwt/user_tab.png)
User details including usage percentage, validity, subscribed plan and other billing details is available in the **USER** tab. You can also download the invoice.
